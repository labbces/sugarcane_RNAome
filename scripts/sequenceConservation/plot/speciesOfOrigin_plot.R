...
countsOrigin<-read.delim(file = "speciesOfOriginPanTranscriptome_FPKM.tsv",header=T,row.names=1)
head(countsOrigin)
...
quantile(countsOrigin$FPKM_SBAR, c(.1,.2,.5,.8,.9,.99))
quantile(countsOrigin$FPKM_SSPO, c(.1,.2,.5,.8,.9,.99))
quantile(countsOrigin$FPKM_SOFF, c(.1,.2,.5,.8,.9,.99))
...
xy_mappings <- list(
  filtered_origin_SOFF_SSPO = c("FPKM_SOFF", "FPKM_SSPO"),
  filtered_origin_SOFF_SBAR = c("FPKM_SOFF", "FPKM_SBAR"),
  filtered_origin_SSPO_SBAR = c("FPKM_SSPO", "FPKM_SBAR")
)
