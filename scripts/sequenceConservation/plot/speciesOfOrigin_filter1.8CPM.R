...
tx2gene <- read.table(file.path(DIR, "updated_panTranscriptome_panRNAome_GeneFunction_Length.tsv"), header = FALSE, sep = "\t")

# teste merge
countsOrigin <- merge(countsOrigin, tx2gene, by.x = "row.names", by.y = "V3")
colnames(countsOrigin) <- c("Transcript","S._barberi","S._officinarum","S._spontaneum","S._bicolor", "Category", "Gene", "Function", "Transcript Size", "Transcript Function")
...
filtered_origin_SOFF_SSPO_noncoding <- subset(filtered_origin_SOFF_SSPO, Function == "ncRNA")
filtered_origin_SOFF_SSPO_lncRNA <- subset(filtered_origin_SOFF_SSPO, Function == "lncRNA")
...
#################################### SOFF and SSPO - lncRNA

# Contagem de transcritos por função
function_counts <- filtered_origin_SOFF_SSPO_lncRNA %>%
  group_by(Origin) %>%
  summarise(Count = n())
...
# Gráfico de dispersão
scatter_plot <- ggplot(as.data.frame(filtered_origin_SOFF_SSPO_lncRNA), aes(x=CPM_SOFF, y=CPM_SSPO)) +
  theme_minimal(base_size = 20) +
  theme(legend.position = "none") +
  geom_jitter(aes(colour=Origin), alpha=0.2, size=1.5) +
  scale_color_brewer(palette="Set1") +
...
# Salvar a combinação como PNG
ggsave("common_SOFF_SSPO_lncRNA.png", plot = final_plot, width = 18, height = 11, dpi = 500, bg="white")
...
filtered_origin_SOFF_SBAR_noncoding <- subset(filtered_origin_SOFF_SBAR, Function == "ncRNA")
filtered_origin_SOFF_SBAR_lncRNA <- subset(filtered_origin_SOFF_SBAR, Function == "lncRNA")
...
# Gráfico de dispersão
scatter_plot <- ggplot(as.data.frame(filtered_origin_SOFF_SBAR_lncRNA), aes(x=CPM_SOFF, y=CPM_SBAR)) +
  theme_minimal(base_size = 20) +
  theme(legend.position = "none") +
  geom_jitter(aes(colour=Origin), alpha=0.2, size=1.5) +
  scale_color_brewer(palette="Set1") +
...
# Salvar a combinação como PNG
ggsave("common_SOFF_SBAR_lncRNA.png", plot = final_plot, width = 18, height = 11, dpi = 500, bg="white")
...
filtered_origin_SSPO_SBAR_noncoding <- subset(filtered_origin_SSPO_SBAR, Function == "ncRNA")
filtered_origin_SSPO_SBAR_lncRNA <- subset(filtered_origin_SSPO_SBAR, Function == "lncRNA")
...
# Gráfico de dispersão
scatter_plot <- ggplot(as.data.frame(filtered_origin_SSPO_SBAR_lncRNA), aes(x=CPM_SSPO, y=CPM_SBAR)) +
  theme_minimal(base_size = 20) +
  theme(legend.position = "none") +
  geom_jitter(aes(colour=Origin), alpha=0.2, size=1.5) +
  scale_color_brewer(palette="Set1") +
...
# Salvar a combinação como PNG
ggsave("common_SSPO_SBAR_lncRNA.png", plot = final_plot, width = 18, height = 11, dpi = 500, bg="white")
