...
tx2gene_protein_coding <- subset(tx2gene, V4 == "protein-coding")
tx2gene_protein_coding <- tx2gene_protein_coding[, c(3,2)]
print("New tx2gene -> transcript ID    group")
tx2gene_protein_coding

# *** Import quantification matrix with tximport ***
txi <- tximport(files, type = "salmon", tx2gene = tx2gene_protein_coding)
...
write.table(counts_matrix_vst_top20, file = "Hoang2017_counts_filters_VST_top20CV.txt", sep = "\t", quote = FALSE)
...
write.table(cv_after_cv_filter, file = "Hoang2017_top20CV.txt", sep = "\t", quote = FALSE)
...
pca_sugar_content_plot <- ggplot(pca_scores, aes(x = PC1, y = PC2, color = ddsColl_top_20_percent$sugar_content, shape = internode_type, label = ddsColl_top_20_percent$genotype)) +
  geom_point(size = 2) +
  geom_text_repel(
    box.padding = 0.1, point.padding = 0.1,
    segment.color = "black", segment.size = 0.1, segment.alpha = 0.5, max.overlaps = 15,
  ...
  labs(title = "PCA - Hoang2017 Contrasting Genotypes in Fiber and Sugar",
       ...
       color = "Groups",
       shape = "Internode Type") +
  stat_ellipse(geom = "polygon", level=0.95, alpha=0.1, aes(fill = ddsColl_top_20_percent$sugar_content), color=NA, show.legend = FALSE) + # add ellipse with 95% confidence intervals
  theme_classic() +
  theme(
    axis.line = element_blank(),  # Linha dos eixos X e Y
    panel.grid.major = element_line(color = alpha("gray", 0.2)),  # Remover linhas de grade principais
