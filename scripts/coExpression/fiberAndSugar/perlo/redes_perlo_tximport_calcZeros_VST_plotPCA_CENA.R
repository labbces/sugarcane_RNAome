...
# *** Configure directory ***
HOME_DIR = "/Storage/data1/felipe.peres/Sugarcane_ncRNA/9_Fiber_and_Sugar/co-expression/Perlo/code"
setwd(HOME_DIR)
...
print('dim(counts(dds_after_filters))')
...
# *** Plot a histogram of the Coefficient of Variation (CV) ***
hist(cv_after_cv_removal, breaks = 50, main = "Coefficient of Variation Distribution after Degraded Samples, Zeros and Low CV Removal",
     xlab = "Coefficient of Variation (%)", ylab = "Frequency")
...
colors <- viridis::viridis(70) #40 cores

# *** Adicione uma coluna ao seu DataFrame de amostras indicando o internode_type ***
dds_vst$internode_type <- sub("^.*_(.+)_.*$", "Internode_\\1", dds_vst$Run)
...
pca_plot <- ggplot(pca_data, aes(x = PC1, y = PC2, color = dds_vst$internode_type, label = dds_vst$genotype)) +
  geom_point(size = 2) +
  geom_text_repel(
  ...
  labs(title = "PCA - Perlo2022 Contrasting Genotypes in Fiber and Sugar",
       ...
       color = "Internode Type",
       shape = "Internode Type") +
  theme_minimal()

# *** Saving PCA ***
#print(pca_plot) 
print("saving PCA as: plot_pca_vst_withInternodeType.png")
ggsave("plot_pca_vst_withInternodeType.png", pca_plot, bg = "white")
...
#print(pca_plot_se)
ggsave("plot_pca_SE_withoutInternodeType.png", pca_plot_se, bg = "white")