...
annotation_df.columns = ['panRNAome_category', 'gene_name', 'transcript_name', 'gene_function', 'transcript_length', 'transcript_function', 'Transcript Rfam family', 'Gene Rfam family']
...
#GO_df = pd.read_csv(GO_file, sep='\t', header=0, dtype=str)
# convertendo colunas para os tipos corretos
#GO_df = GO_df.drop(0)
#GO_df['ARGOT_score'] = GO_df['ARGOT_score'].astype(float)
#GO_df['ARGOT_PPV'] = GO_df['ARGOT_PPV'].astype(float)
#GO_df['ARGOT_rank'] = GO_df['ARGOT_rank'].astype(int)
#GO_df['goclasscount'] = GO_df['goclasscount'].astype(int)
